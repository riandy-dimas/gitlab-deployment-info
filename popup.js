...
import { summarizeCommits, checkAIAvailability, cleanupSummarizer, getDownloadConfirmationContent  } from "./utils/ai.js";
...
document.getElementById('summarizeBtn').addEventListener('click', async () => {
  try {
    ...
    // Try to summarize (first attempt without confirmation)
    let result = await summarizeCommits(commits, false);
    ...
    if (result.needsConfirmation) {
      ...
      result = await summarizeCommits(commits, true);
    }
    ...
    if (result.result) {
      showAISummaryModal(result.result);
    }
...
function showAISummaryModal(summary) {
  ...
  aiMethod.textContent = 'Generated by Gemini Nano';
