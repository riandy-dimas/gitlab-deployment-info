      ...
      if (key.startsWith(CACHE_PREFIX)) {
        try {
          ...
          if (now - data.timestamp > CACHE_MAX_AGE) {
            ...
            removed++;
            console.log(
              `🗑️ Removed expired cache: ${data.project} ${data.fromTag} → ${data.toTag}`
            );
          }
        } catch (error) {
          ...
          localStorage.removeItem(key);
          removed++;
          console.log(`🗑️ Removed corrupted cache entry: ${key}`);
        }
      }
    });
...
export async function isSummarizerAvailable() {
  ...
  if (!("Summarizer" in self)) {
    return {
      available: false,
      reason: "Summarizer API not supported in this browser",
    };
  }
    ...
    switch (availability) {
      ...
      case "available":
        return {
          available: true,
          status: "ready",
          reason: "Summarizer API is available",
        };
      default:
        return {
          available: false,
...
async function createSummarizer() {
  const defaultOptions = {
    sharedContext:
      "These are deployment changes from a GitLab repository. " +
...
function truncateCommits(commits, maxChars = 3000) {
  ...
  for (const commit of commits) {
    ...
    const shortCommit =
      commit.length > 200 ? commit.substring(0, 200) + "..." : commit;
...
export async function summarizeCommits(
  ...
  if (!summarizerStatus.available) {
    ...
    if (
      summarizerStatus.status === "after-download" ||
      summarizerStatus.status === "before-download"
    ) {
      if (!userConfirmed) {
      ...
      if (summarizerStatus.status === "after-download") {
        throw new Error(
          "AI model is still downloading. Please wait a few minutes and try again. Check progress at chrome://on-device-internals/"
        );
      }
      ...
      throw new Error(
        `Summarizer API not available: ${summarizerStatus.reason}`
      );
    }
  }
    ...
    if (commits.length > 20) {
      ...
      if (summaries.length > 1) {
        ...
        finalSummary = await summarizer.summarize(combinedSummary, {
          context:
            "These are summarized deployment changes from multiple commit summaries. Provide a concise overall summary. This is project repository for " +
            project +
            ".",
        });
      } else {
      ...
      console.log(
        `Summarizing ${truncatedCommits.length} commits (${text.length} chars)`
      );

      // Summarize the text
      finalSummary = await summarizer.summarize(text, {
        context:
          "These are truncated commits that were shortened to first 200 characters. Provide a concise overall summary. This is project repository for " +
          project +
          ".",
      });
    }
